...
use crate::execution::resolve_execution_cwds;
use crate::protocol::*;
use crate::service::{
...
fn inline_bindings_prefix(bindings: &BTreeMap<String, RegisterHostCallbacksRequest>) -> String {
    ...
    format!("{DEFINE_GLOBALS}({});", serde_json::Value::Object(groups))
}
...
fn resolve_inline_guest_file_path(file_path: &str, guest_cwd: &str) -> String {
    if file_path.starts_with('/') {
        normalize_path(file_path)
    } else {
        let base = guest_cwd.trim_end_matches('/');
    ...
    async fn execute_language_operation_owned(
            ...
            && operation.args.first().map(String::as_str) == Some("-e")
        {
            ...
            if !prefix.is_empty() {
                ...
                if let Some(source) = operation.retained_source.as_mut() {
                    source.insert_str(0, &prefix);
                }
            }
        }
        ...
        if operation.command == "__agentos_typescript_file" {
            ...
            let (guest_cwd, _, _) = resolve_execution_cwds(&vm, operation.cwd.as_deref());
        ...
        if operation.env.contains_key(INLINE_FILE_PATH_ENV) {
            ...
            let inline_path = operation
                ...
                .clone();
            let resolved = resolve_inline_guest_file_path(&inline_path, &guest_cwd);
            operation
                .env
...
mod inline_bindings_prefix_tests {
    ...
    fn defines_one_group_per_collection_on_a_single_line() {
        ...
        assert!(prefix.ends_with(
            r#"({"orderStore":{"command":"agentos-order-store","functions":[["listOrders","list-orders"]]}});"#
        ));
    }
}
